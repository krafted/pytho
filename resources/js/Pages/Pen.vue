<template>
    <l-app>
        <template #header-right-actions>
            ...
            <x-details v-if="!isCreator && pen" :pen="pen" />
        </template>
...
export default {
    ...
    setup(props) {
        ...
        const message = ref("");
        const closeCanvas = () =>
            form.value.content.includes("exitonclick") &&
        ...
        const run = async () => {
            ...
            if (
                ["import turtle", "from turtle import"].some((s) =>
                    form.value.content.includes(s)
                )
            ) {
                await loadEngine("skulpt");
                showCanvas.value = true;
                await nextTick();
                await runCode(form.value.content, {
                    use: "skulpt",
                    canvas: "canvas",
                });
        ...
        useHead({
            title: computed(() =>
                props.pen ? `${props.pen?.title} — Pytho` : "Pytho"
            ),
        });
        ...
        onUnmounted(() =>
            hotkeys.unbind(isMac.value ? "cmd+enter" : "ctrl+enter")
        );
        ...
        onMounted(async () => {
            ...
            setOptions({
                ...
                error: (err) => (
                    (error.value = true),
                    (output.value += err.error?.message ?? err.error)
                ),
            });
