<template>
    ...
    >
        ...
        <template #title> Leave a Reply </template>

        <template #content>
            <f-textarea label="Text" ref="comment" v-model="commentForm.body" />
            <f-input-error :message="commentForm.errors.text" />
        </template>

        <template #actions>
            <x-secondary-button type="button" @click="showCommentForm = false">
                Close
            </x-secondary-button>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const showComment = async () => {
            commentForm.value.properties.coords = [
                editor.value.getCursor(true),
                editor.value.getCursor(false),
            ];
        ...
        const onCursorActivity = async (e) => {
            ...
            setTimeout(() => {
                ...
                if (!button) {
                    ...
                    button.className =
                        "comment-button uppercase tracking-wide shadow font-semibold text-sm rounded-md absolute z-10 p-2.5 bg-white dark:bg-gray-1000 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500";
                ...
                if (selected)
                    createPopper(selected, button, { placement: "top" });
            }, 0);
        };
        const saveComment = () => {
            if (!props.pen) return;

            commentForm.value.post(route("pen.comments.store", props.pen), {
                preserveState: false,
            });
        };
        ...
        onMounted(() => {
            ...
            Object.entries(props.comments).forEach((line) => {
                ...
                button.classList =
                    "text-gray-400 dark:text-gray-600 hover:text-gray-600 dark:hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500";
                ...
                editor.value.addLineWidget(parseInt(n), button, {
                    className:
                        "z-10 absolute top-0 right-3 -mr-px h-full flex items-center justify-center",
                });
            });
        });
